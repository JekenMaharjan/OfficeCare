...
const Products = () => {
    ...
    const deleteProduct = async (id: string) => {
        try {
            const { data } = await axios.delete(`${process.env.NEXT_PUBLIC_API_URL}/Product/${id}`);
            toast(data);
    ...
    const getTotalInventoryValue = (products: Product[]) => {
        return products.reduce((total, product) => {
            const price = Number(product.price) || 0; // Ensure numeric
            const stock = Number(product.stock) || 0;
            return total + price * stock;
        }, 0);
    };

    // Calculate low stock items (<= 5)
    const getLowStockCount = (products: Product[]) => {
        return products.filter(product => Number(product.stock) <= 5).length;
    }
                ...
                key={product._id} className="grid grid-cols-6 gap-5 p-4 items-center border-b hover:bg-accent/50 transition-colors">
                    ...
                    <div className="ml-12">
                        <Badge variant={getStatusColor(
                            product.stock <=0 ? "inactive" : product.stock <=5 ? "low_stock" : "active",
                            product.stock
                            )}>
                            {getStatusText(
                                product.stock <=0 ? "inactive" : product.stock <=5 ? "low_stock" : "active", 
                    ...
                    <Button variant="ghost" size="sm" className="cursor-pointer hover:bg-gray-200">
                        <Eye className="h-4 w-4" />
                    </Button>
                    <Button variant="ghost" size="sm" className="cursor-pointer hover:bg-gray-200">
                        <Edit className="h-4 w-4" />
                    </Button>
                    <Button onClick={() => deleteProduct(product._id)} variant="ghost" size="sm" className="cursor-pointer hover:bg-gray-200">
